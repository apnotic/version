...
Prerelease versions are formatted with a dash separator (e.g., 1.0.0-a instead of 1.0.0a).
...
Now `rake -T version` will tell you what all you can do.
  ...
  rake version:bump:pre           # Bump to 0.1.1-a
  rake version:bump:pre:major     # Bump to 1.0.0-a
  rake version:bump:pre:minor     # Bump to 0.2.0-a
  rake version:bump:pre:revision  # Bump to 0.1.1-a
...
If you prefer to add a prefix to the git tag:
