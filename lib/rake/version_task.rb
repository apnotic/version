...
class Rake::VersionTask < Rake::TaskLib
  ...
  # When set, prepends this string to the version before tagging (default: nil)
  attr_accessor :git_tag_prefix
  ...
  def write(version)
    ...
    if self.with_git
      ...
      if self.with_git_tag
        if self.git_tag_prefix
          `git tag #{self.git_tag_prefix}#{version}`
        else
          `git tag #{version}`
        end
      end
